...
const highlighter = options => tree => {
    ...
    const fontFamily = monospacedFont ? "font-family: monospace, monospace;" : ""
            ...
            match => {
                switch(displayMode) {
                    case coloredTextMode:
                        return `<span style="color: ${match}; ${fontFamily}">${match}</span>`
                    case coloredTextBackgroundMode:
                        return `<span style="color: ${match}; background-color: ${backgroundColor}; ${fontFamily} border-radius: 2px; padding-left: 3px; padding-right: 3px;">${match}</span>`
                    case coloredBackgroundMode:
                        return `<span style="background-color: ${match}; ${fontFamily} border-radius: 2px; padding-left: 3px; padding-right: 3px;">${match}</span>`
                    default:
